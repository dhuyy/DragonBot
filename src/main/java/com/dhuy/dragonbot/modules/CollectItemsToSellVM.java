...
public class CollectItemsToSellVM {
  ...
  public CollectItemsToSellVM() throws AWTException {
    log = Log.getInstance();
    xml = new XMLHelper();
    mouse = new Mouse();
    ...
    try {
      ...
      while (shouldCollectMoreItems) {
        if (shouldOpenDepotBox) {
          mouse.clickOn(MouseCoordinatesVM.DEPOT_BOX_X,
              MouseCoordinatesVM.DEPOT_BOX_Y + store.getWindowsTitleBarHeight(), true);
          delay(500);

          mouse.clickOn(MouseCoordinatesVM.SEARCH_ITEMS_BUTTON_X,
              MouseCoordinatesVM.SEARCH_ITEMS_BUTTON_Y, store.getWindowsTitleBarHeight());
          delay(500);

          shouldOpenDepotBox = false;
        }

        mouse.clickOn(MouseCoordinatesVM.CLEAR_FOUND_ITEM_NAME_X,
            MouseCoordinatesVM.CLEAR_FOUND_ITEM_NAME_Y, store.getWindowsTitleBarHeight());
        delay(500);

        keyboard.writeWord(items.get(store.getCurrentCollectItemIndex()).getName());
        delay(500);

        mouse.clickOn(MouseCoordinatesVM.FIRST_FOUND_ITEM_X, MouseCoordinatesVM.FIRST_FOUND_ITEM_Y,
            store.getWindowsTitleBarHeight());
        delay(500);

        mouse.clickOn(MouseCoordinatesVM.SEARCH_FOR_ITEMS_X, MouseCoordinatesVM.SEARCH_FOR_ITEMS_Y,
            store.getWindowsTitleBarHeight());
        delay(500);
        ...
        String isThereAnyItemPixelHex = imageProcessor.getHexFromColor(
            new Color(currentScreenshot.getSubimage(MouseCoordinatesVM.CHECK_EXISTING_ITEMS_X,
                MouseCoordinatesVM.CHECK_EXISTING_ITEMS_Y, 1, 1).getRGB(0, 0)));
        ...
        if (isThereAnyItemPixelHex
          ...
          while (isThereCapacityLeft && isThereAnyItemPixelHex
              .equals(Store.INDICATOR_EXISTING_ITEMS_IN_POSTAL_BOX_PIXEL_COLOR)) {
            mouse.clickOn(MouseCoordinatesVM.RETRIEVE_ITEMS_X, MouseCoordinatesVM.RETRIEVE_ITEMS_Y,
                store.getWindowsTitleBarHeight());
                    ...
                    .getHexFromColor(
                        new Color(
                            newCurrentScreenshot
                                .getSubimage(MouseCoordinatesVM.PIXEL_NOT_ENOUGH_CAPACITY_X,
                                    MouseCoordinatesVM.PIXEL_NOT_ENOUGH_CAPACITY_Y, 1, 1)
                                .getRGB(0, 0)));

            if (isThereCapacityLeftPixelHex.equals(Store.NOT_ENOUGH_CAPACITY_PIXEL_COLOR)) {
              mouse.clickOn(MouseCoordinatesVM.NOT_ENOUGH_CAPACITY_INFO_MESSAGE_X,
                  MouseCoordinatesVM.NOT_ENOUGH_CAPACITY_INFO_MESSAGE_Y,
                  store.getWindowsTitleBarHeight());
              delay(500);
            ...
            isThereAnyItemPixelHex = imageProcessor.getHexFromColor(
                new Color(newCurrentScreenshot.getSubimage(MouseCoordinatesVM.CHECK_EXISTING_ITEMS_X,
                    MouseCoordinatesVM.CHECK_EXISTING_ITEMS_Y, 1, 1).getRGB(0, 0)));
          }
          ...
          if (!isThereAnyItemPixelHex
            ...
            mouse.clickOn(MouseCoordinatesVM.GO_BACK_SEARCH_ITEMS_X,
                MouseCoordinatesVM.GO_BACK_SEARCH_ITEMS_Y, store.getWindowsTitleBarHeight());
            delay(500);
          }
